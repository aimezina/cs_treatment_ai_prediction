...
@tf.function()
def AsymmetricLossOptimized(y_true, y_pred):
    gamma_neg = 5
    gamma_pos = 0.5
    clip = 0.01
    eps = 1e-8
    ...
    if clip is not None and clip > 0:
        xs_neg = tf.clip_by_value(tf.math.add(xs_neg, clip), clip_value_min=min(tf.math.add(xs_neg, clip)),
                                  clip_value_max=1)

        # Basic CE calculation
    # loss = targets * log(tf.clip_by_value(xs_pos, clip_value_min=eps, clip_value_max=max(xs_pos)))
    # loss = tf.math.add(loss,
    #                    anti_targets * log(tf.clip_by_value(xs_neg, clip_value_min=eps, clip_value_max=max(xs_neg))))
